...
class Flow(object):
    ...
    def __init__(self,main='payload'):
        ...
        self.lengths = self.frame_lengths if main == 'frame.len' else (self.payload_lengths if main == 'payload' else self.ip_lengths)
    ...
    def add(self, packet,extension):
        ...
        self.ip_lengths   .append( int(packet[4]) if (packet[5], int(packet[7])) == (self.src, self.sport) else
                               -int(packet[4]))
        self.frame_lengths.append(int(packet[-2]) if (packet[5], int(packet[7])) == (self.src, self.sport) else
                               -int(packet[-2]))
        if int(packet[9]) != 0:
            try:
