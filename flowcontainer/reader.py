...
class Reader(object):
    ...
    def read_tshark(self, path,filter_str="",extension="",ip_layer =False, cmd_parameter=[]):
        ...
        if ip_layer == False:
            command = ["tshark", "-r", path, "-Tfields", "-E", "separator=`", "-o", "tcp.desegment_tcp_streams:TRUE",
                   "-e", "frame.time_epoch",
                   "-e", "tcp.stream",
            ...
            command = ["tshark", "-r", path, "-Tfields", "-E", "separator=`", "-o", "TCP.REASEMBLY.ENABLE_TCP_STREAMS:TRUE",
                   "-e", "frame.time_epoch",
                   "-e", "tcp.stream",
                   "-e", "udp.stream", #only output one line
                   "-e", "ip.proto",
        ...
        for packet in filter(None, out.decode('utf-8',errors="ignore").split('\n')):
            ...
            #路径|tcp(udp)|flowid|时间戳|IP长度|srcIP|dstIP|srcport|dstport|payload长度|frame长度|extension
            timestamp=packet[0]
            ext_protocol = packet[18]
            extension = packet[19:-1]
            ...
            if protocol in ['tcp','udp'] and ip_layer == False:
                ...
                dstport = 0
                payload_length = 0
            result.append([path]+[(protocol,ext_protocol), flowid, timestamp, ip_len, ip_src, ip_dst, srcport, dstport,payload_length, frame_len, extension])
